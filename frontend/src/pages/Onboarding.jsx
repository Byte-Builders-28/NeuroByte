...
colors={[
  ...
  0x98FB98
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full px-4">
            {features.map((feature, idx) => (
              ...
              >
                ...
                <h3 className="relative z-10 text-xl md:text-2xl font-bold text-slate-800 mb-4 tracking-tight transition-all duration-300">{feature.title}</h3>
                <p className="relative z-10 text-slate-600 leading-relaxed text-base font-medium">{feature.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ===== MODAL (outside .glass so position:fixed works relative to viewport) ===== */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          >
            ...
            <div className="relative z-20 flex justify-end px-8 pt-8 pb-2 shrink-0">
              <button onClick={() => setIsModalOpen(false)} className="p-2 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors">
                <X size={24} />
              </button>
            </div>
